...
public class Row {
  ...
  // column is not accessible from outside, this method guarantees only one thread can changed
  // column at a time
  ...
  private boolean updateSingleColumnWithoutLock(
      String columnName, UpdateColumnInterface updateOperation) {
    Column prev = columns.get(columnName);
    if (Optional.ofNullable(prev).isEmpty()) return false;
  ...
  // readSingleColumnWithoutLock can only be used inside write-lock block. This Method returns
  // either a copy of ori-column or null
