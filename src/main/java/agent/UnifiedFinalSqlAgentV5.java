 ...
 * -Dsql.agent.hook.DriverManager=true \
 * -Dsql.agent.log.mybatis=true \
 * -Dsql.agent.log.jdbc=true \
 * -Dsql.agent.log.errors=true \
 * -Dsql.agent.prefer.mybatis=true \
 * -Dsql.agent.sample=1.0 \
 * -Dsql.agent.dedupe.window.ms=500 \
 * -Dsql.agent.sql.ignore="(?i)^\\s*select\\s+1(\\s+from\\s+dual)?\\s*$;(?i)^\\s*values\\s+1\\s*$" \
 * -jar your-app.jar
 * <p>
 * SECURITY NOTE:
 * Exporting credentials is dangerous. Password logging is OFF by default and masked when enabled.
 */
public final class UnifiedFinalSqlAgentV5 {
    private UnifiedFinalSqlAgentV5() {
    }
    ...
    public static void premain(String args, Instrumentation inst) {
        ...
        try {
            for (Class<?> c : inst.getAllLoadedClasses()) {
                ...
                String n = c.getName();
                if (n.startsWith("org.apache.ibatis.executor.") || implementsAnyJdbc(c) || "java.sql.DriverManager".equals(n)) {
                    try {
    ...
    static final class Xf implements ClassFileTransformer {
        ...
        public byte[] transform(ClassLoader loader, String className, Class<?> cls, ProtectionDomain pd, byte[] buf) {
            ...
            // Allow transforming DriverManager when explicitly enabled
            boolean hookDM = Boolean.getBoolean("sql.agent.hook.DriverManager");
            if ((n.startsWith("java.") || n.startsWith("javax.") || n.startsWith("jdk.") || n.startsWith("sun.") || n.startsWith("com.sun."))
                    && !(hookDM && "java.sql.DriverManager".equals(n))) {
                return null;
            }
            ...
            try {
                ...
                // JDBC: Connection / PreparedStatement / Statement / Driver / DriverManager
                ...
                if (subtypeOf(cp, ct, "java.sql.Driver")) mod |= instrumentConnectors(ct, "connect");
                if (hookDM && "java.sql.DriverManager".equals(n)) mod |= instrumentConnectors(ct, "getConnection");
                if (subtypeOf(cp, ct, "javax.sql.DataSource")) mod |= instrumentDataSource(ct);
                if (n.startsWith("org.apache.ibatis.transaction.")) mod |= instrumentMyBatisTransaction(ct);
        ...
        boolean instrumentDataSource(CtClass ct) throws Exception {
            boolean mod = false;
            for (CtMethod m : ct.getDeclaredMethods()) {
                if (!"getConnection".equals(m.getName())) continue;
                try {
                    if (!"java.sql.Connection".equals(m.getReturnType().getName())) continue;
                ...
                if (p.length == 0) {
                    try {
                        m.insertAfter("{ if ($_ != null) " + UnifiedFinalSqlAgent.class.getName() +
                                ".DriverSpecific.registerFromDataSource($_, $0); }", false);
                        mod = true;
                    } catch (CannotCompileException ignored) {
                    }
                }
                // getConnection(String user, String password)
                if (p.length == 2 && "java.lang.String".equals(p[0].getName()) && "java.lang.String".equals(p[1].getName())) {
                    try {
                        m.insertAfter("{ if ($_ != null) " + UnifiedFinalSqlAgent.class.getName() +
                                ".DriverSpecific.registerFromDataSource($_, $0); }", false);
                        mod = true;
        ...
        boolean instrumentMyBatisTransaction(CtClass ct) throws Exception {
            boolean mod = false;
            // Any MyBatis transaction impl that exposes getConnection(): JdbcTransaction, ManagedTransaction, SpringManagedTransaction
            for (CtMethod m : ct.getDeclaredMethods()) {
                if (!"getConnection".equals(m.getName())) continue;
                try {
                    if (!"java.sql.Connection".equals(m.getReturnType().getName())) continue;
        ...
        /**
         * Unifies instrumentation for Driver.connect and DriverManager.getConnection.
         */
        boolean instrumentConnectors(CtClass ct, String methodName) throws Exception {
            boolean mod = false;
            for (CtMethod m : ct.getDeclaredMethods()) {
                ...
                if (p.length == 2
                        && "java.lang.String".equals(p[0].getName())
                        && "java.util.Properties".equals(p[1].getName())) {
                    try {
                        m.insertAfter(
                                "{ if ($_ != null) " + UnifiedFinalSqlAgent.class.getName() +
                                        ".DriverSpecific.registerFromDriver($_, $1, (java.util.Properties)$2); }",
                                false
                        );
                        mod = true;
                    } catch (CannotCompileException ignored) {
                ...
                // (String url, String user, String password) -- DriverManager only
                if (p.length == 3
                        && "java.lang.String".equals(p[0].getName())
                        && "java.lang.String".equals(p[1].getName())
                        && "java.lang.String".equals(p[2].getName())) {
                    try {
                        m.insertAfter(
                                "{ if ($_ != null) { " +
                                        "    java.util.Properties __p = new java.util.Properties(); " +
                                        "    if ($2 != null) __p.setProperty(\"user\", $2); " +
                                        "    if ($3 != null) __p.setProperty(\"password\", $3); " +
                                        UnifiedFinalSqlAgent.class.getName() +
                                        ".DriverSpecific.registerFromDriver($_, $1, __p); " +
                                        "} }",
                                false
                        );
                        mod = true;
    ...
    static final class MyBatisCtx {
        ...
        static void enter(Object mappedStatement, Object paramObj) {
            ...
            c.enterMs = System.currentTimeMillis();
            c.active = true;
            try {
        ...
        static void exit() {
            ...
            String json = Json.start()
                    ...
                    .end();
            if (Filter.LOG_MYBATIS && Filter.allowMyBatis(c.finalSql)) {
                Log.json(json);
            }
    ...
    public static void onJdbcExecute(Object self, String method, Object[] args) {
        try {
            if (!Filter.LOG_JDBC) return;
            MyBatisCtx.Ctx ctxCheck = MyBatisCtx.TL.get();
            if (Filter.PREFER_MYBATIS && ctxCheck != null && ctxCheck.active) return;
            ...
            try {
                if (conn != null) ConnRegistry.register(conn);
            } catch (Throwable ignored) {
            }
            ...
            if (!Filter.allowJdbc(sql, finalSql)) return;
            ...
            String json = Json.start()
                    ...
                    .kv("connClass", conn == null ? null : conn.getClass().getName())
                    .kv("connId", conn == null ? null : String.valueOf(System.identityHashCode(conn)))
            ...
            // Optional debug: report missing password when requested
            try {
                boolean dbg = Boolean.parseBoolean(System.getProperty("sql.agent.debug.credentials", "false"));
                boolean wantPwd = Boolean.parseBoolean(System.getProperty("sql.agent.export.password", "false"));
                if (dbg && wantPwd && (ci == null || ci.password == null)) {
                    Log.json("{\"type\":\"agent-warn\",\"msg\":\"password-not-found\",\"driverClass\":\"" + esc(self.getClass().getName()) + "\"," +
                            "\"url\":\"" + esc(ci == null ? null : ci.url) + "\"}");
                }
            } catch (Throwable ignored) {
            }
    ...
    static final class DriverSpecific {
        ...
        private static final Map<Object, ConnInfo> CONN_CREDS = Collections.synchronizedMap(new WeakHashMap<>());

        static void registerFromDriver(java.sql.Connection c, String url, java.util.Properties props) {
            try {
                if (c == null) return;
                ConnInfo ci = new ConnInfo();
                ci.url = url;
                if (props != null) {
                    ci.user = props.getProperty("user", props.getProperty("username"));
                    ci.password = props.getProperty("password", props.getProperty("pwd", props.getProperty("pass")));
                }
                CONN_CREDS.put(c, ci);
                ConnRegistry.register(c);
            } catch (Throwable ignored) {
            }
        }

        static void registerFromDataSource(java.sql.Connection c, Object ds) {
            try {
                if (c == null) return;
                ConnRegistry.register(c, ds);
                ConnInfo existing = CONN_CREDS.get(c);
                ConnInfo fromDs = readConnInfoFromDataSource(ds);
                if (existing != null || fromDs != null) CONN_CREDS.put(c, merge(existing, fromDs));
            } catch (Throwable ignored) {
            }
        }
        ...
        static ConnInfo readConnInfoFromDataSource(Object ds) {
            ...
            try {
                ...
                // Spring wrappers: unwrap to target DataSource
                if (out.url == null || out.user == null || out.password == null) {
                    try {
                        String cn = ds.getClass().getName();
                        if (cn.startsWith("org.springframework.jdbc.datasource.")) {
                            Object target = null;
                            target = (target == null ? callNoArg(ds, "getTargetDataSource") : target);
                            if (target == null) target = findFieldByName(ds, "targetDataSource", "dataSource");
                            if (target == null) {
                                try {
                                    java.lang.reflect.Method m = ds.getClass().getDeclaredMethod("determineTargetDataSource");
                                    m.setAccessible(true);
                                    target = m.invoke(ds);
                                } catch (Throwable ignored2) {
                                }
                            }
                            if (target != null && target != ds) {
                                ConnInfo deeper = readConnInfoFromDataSource(target);
                ...
                // Generic properties bags (Druid/DBCP/etc.)
                if (out.password == null || out.user == null) {
                    try {
                        Object props = callNoArg(ds, "getConnectProperties"); // Druid
                        if (!(props instanceof java.util.Properties))
                            props = callNoArg(ds, "getConnectionProperties"); // DBCP
                        if (props instanceof java.util.Properties) {
                            java.util.Properties pr = (java.util.Properties) props;
                            if (out.password == null)
                                out.password = pr.getProperty("password", pr.getProperty("pwd", pr.getProperty("pass")));
                            if (out.user == null) out.user = pr.getProperty("user", pr.getProperty("username"));
                            if (out.url == null) out.url = pr.getProperty("url", out.url);
                        }
                    } catch (Throwable ignored) {
                    }
                }
        ...
        /**
         * Best-effort extraction of connection URL/username/[optional] password.
         */
        static ConnInfo readConnInfo(java.sql.Connection c) {
            ...
            try {
                ConnInfo fromDriver = CONN_CREDS.get(c);
                ci = merge(ci, fromDriver);
            } catch (Throwable ignored) {
            }
            ...
            if (c != null) {
                try {
                    Object ds = ConnRegistry.lookupDataSource(c);
                    if (ds == null) ds = findDataSourceFromConnection(c, 0, new java.util.HashSet<>());
                    ConnInfo fromDs = readConnInfoFromDataSource(ds);
                    ci = merge(ci, fromDs);
                } catch (Throwable ignored) {
                }
            ...
            if (ci.password != null && (!wantPwd || mask)) ci.password = maskRep;
            return ci;
        }
    ...
    static final class ConnRegistry {
        static final ThreadLocal<java.sql.Connection> LAST = new ThreadLocal<>();
        static final Map<java.sql.Connection, Object> CONN_TO_DS = Collections.synchronizedMap(new WeakHashMap<>());

        static void register(java.sql.Connection c) {
            if (c != null) LAST.set(c);
        }

        static void register(java.sql.Connection c, Object ds) {
            if (c != null) {
                LAST.set(c);
                if (ds != null) CONN_TO_DS.put(c, ds);
            }
        }

        static java.sql.Connection current() {
            return LAST.get();
        }

        static Object lookupDataSource(java.sql.Connection c) {
            return CONN_TO_DS.get(c);
        }
    }

    static final class Filter {
        static final boolean LOG_JDBC = Boolean.parseBoolean(System.getProperty("sql.agent.log.jdbc", "true"));
        static final boolean LOG_MYBATIS = Boolean.parseBoolean(System.getProperty("sql.agent.log.mybatis", "true"));
        static final boolean LOG_ERRORS = Boolean.parseBoolean(System.getProperty("sql.agent.log.errors", "true"));
        static final boolean PREFER_MYBATIS = Boolean.parseBoolean(System.getProperty("sql.agent.prefer.mybatis", "true"));

        static final double SAMPLE = parseDouble("sql.agent.sample", 1.0);
        static final long DEDUPE_WINDOW_MS = Long.getLong("sql.agent.dedupe.window.ms", 500L);
        static final int DEDUPE_CAP = Integer.getInteger("sql.agent.dedupe.capacity", 2048);

        static final java.util.regex.Pattern[] SQL_IGNORE = compileList(System.getProperty("sql.agent.sql.ignore", ""));
        static final java.util.regex.Pattern[] THREAD_IGNORE = compileList(System.getProperty("sql.agent.thread.ignore", ""));

        static final boolean PRINT_INVOCATION = Boolean.parseBoolean(System.getProperty("sql.agent.invocation", "false"));
        static final int INVOCATION_DEPTH = Integer.getInteger("sql.agent.invocation.depth", 8);
        static final java.util.regex.Pattern[] INVOCATION_IGNORE = compileList(System.getProperty("sql.agent.invocation.ignore",
                "(?i)^(agent\\.|java\\.|javax\\.|sun\\.|com\\.sun\\.|jdk\\.|org\\.apache\\.ibatis\\.|org\\.mybatis\\.|javassist\\.|net\\.bytebuddy\\.|\\$Proxy)"));

        static final Map<String, Long> RECENT = Collections.synchronizedMap(
                new LinkedHashMap<String, Long>(DEDUPE_CAP, 0.75f, true) {
                    @Override
        ...
        static boolean allowJdbc(String sql, String finalSql) {
            if (!LOG_JDBC) return false;
            if (!sample()) return false;
            if (isHeartbeat(sql, finalSql)) return false;
            String text = pick(finalSql, sql);
            if (matchAny(SQL_IGNORE, text)) return false;
            if (matchAny(THREAD_IGNORE, Thread.currentThread().getName())) return false;
            if (dedupe("jdbc", text)) return false;
            return true;
        }

        static boolean allowMyBatis(String finalSql) {
            if (!LOG_MYBATIS) return false;
            if (!sample()) return false;
            if (isHeartbeat(finalSql, finalSql)) return false;
            if (matchAny(SQL_IGNORE, finalSql)) return false;
            if (matchAny(THREAD_IGNORE, Thread.currentThread().getName())) return false;
            if (dedupe("mybatis", finalSql)) return false;
            return true;
        }

        static boolean sample() {
            if (SAMPLE >= 1.0) return true;
            return java.util.concurrent.ThreadLocalRandom.current().nextDouble() < SAMPLE;
        }

        static boolean dedupe(String type, String text) {
            if (text == null) return false;
            long now = System.currentTimeMillis();
            String norm = text.replaceAll("\\s+", " ").trim();
            String key = type + "|" + Integer.toHexString(norm.hashCode());
            Long prev = RECENT.put(key, now);
            return prev != null && (now - prev) < DEDUPE_WINDOW_MS;
        }

        static boolean isHeartbeat(String sql, String finalSql) {
            String s = pick(finalSql, sql);
            if (s == null) return false;
            String t = s.trim().toLowerCase(java.util.Locale.ROOT);
            return t.equals("select 1")
                    || t.equals("select 1 from dual")
                    || t.equals("/* ping */ select 1")
                    || t.equals("values 1")
                    || t.startsWith("/* ping */")
                    || t.startsWith("/*healthcheck*/");
        }

        static boolean matchAny(java.util.regex.Pattern[] ps, String s) {
            if (s == null || ps == null) return false;
            for (java.util.regex.Pattern p : ps) if (p != null && p.matcher(s).find()) return true;
            return false;
        }

        static boolean ignoreInvocationFrame(String frame) {
            return matchAny(INVOCATION_IGNORE, frame);
        }


        static java.util.regex.Pattern[] compileList(String csvOrSemicolons) {
            if (csvOrSemicolons == null || csvOrSemicolons.trim().isEmpty()) return new java.util.regex.Pattern[0];
            String[] parts = csvOrSemicolons.split("[,;]");
            java.util.List<java.util.regex.Pattern> out = new java.util.ArrayList<>();
            for (String raw : parts) {
                String r = raw.trim();
                if (r.isEmpty()) continue;
                try {
                    out.add(java.util.regex.Pattern.compile(r, java.util.regex.Pattern.CASE_INSENSITIVE | java.util.regex.Pattern.DOTALL));
                } catch (Throwable ignored) {
                }
            }
        ...
        static double parseDouble(String k, double def) {
            try {
                return Double.parseDouble(System.getProperty(k, Double.toString(def)));
            } catch (Throwable ignored) {
                return def;
            }
        }
    ...
    static final class Sql {
        static String inlineWithParams(String sql, Map<Integer, Object> params, Vendor vendor) {
            ...
            int max = 0;
            for (Integer i : params.keySet()) if (i != null && i > max) max = i;
    ...
    static final class FinalSqlLog {
        private static Disruptor<LogEvent> disruptor;
        private static RingBuffer<LogEvent> ringBuffer;

        // 1. The Event: A pre-allocated object to carry the log message
        ...
        // 2. The Event Factory: Creates the event objects to fill the RingBuffer
        private static final EventFactory<LogEvent> EVENT_FACTORY = LogEvent::new;

        // 3. The Event Handler (Consumer): Writes the log events to the file
        private static final class LogEventHandler implements EventHandler<LogEvent> {
            private final BufferedWriter writer;

            LogEventHandler(String path) {
                try {
                    File f = new File(path);
                    File dir = f.getParentFile();
                    if (dir != null) dir.mkdirs();
                    this.writer = new BufferedWriter(new FileWriter(f, StandardCharsets.UTF_8, true));
                } catch (IOException e) {
        ...
        static synchronized void init(String path) {
            if (disruptor != null) return; // Already initialized

            ThreadFactory threadFactory = r -> {
                Thread t = new Thread(r, "sql-agent-disruptor-writer");
                t.setDaemon(true);
                return t;
            };

            // RingBuffer size, must be a power of 2
            int bufferSize = 16384; // 2^14

            disruptor = new Disruptor<>(EVENT_FACTORY, bufferSize, threadFactory, ProducerType.MULTI, new com.lmax.disruptor.SleepingWaitStrategy());

            LogEventHandler handler = new LogEventHandler(path);
            disruptor.handleEventsWith(handler);

            // Start the Disruptor, and get the RingBuffer
            ringBuffer = disruptor.start();

            // Add a shutdown hook to ensure logs are flushed on exit
        ...
        // This is the Producer method called by the agent
        static void json(String line) {
            // Ensure initialization has happened.
            if (ringBuffer == null) {
                System.err.println("SQL Agent Logger not initialized. Log line lost: " + line);
                return;
            }

            // Publish the log line to the RingBuffer. This is a non-blocking, lock-free operation.
    ...
    static String invocationStack() {
        if (!Filter.PRINT_INVOCATION) return null;
        try {
            ...
            for (StackTraceElement ste : st) {
                ...
                String frame = ste.toString();
                // Skip frames inside agent and JDK/instrumentation or those explicitly ignored
                if (cls.equals(Thread.class.getName())) continue;
                if (Filter.ignoreInvocationFrame(cls) || Filter.ignoreInvocationFrame(frame)) continue;
                if (sb.length() > 0) sb.append("\n\tat ");
                else sb.append("at ");
                sb.append(frame);
                if (++count >= Filter.INVOCATION_DEPTH) break;
            }
