 ...
 *   -jar your-app.jar
 */
public final class UnifiedFinalSqlAgentOri {
    private UnifiedFinalSqlAgentOri() {}
    ...
    static final class Sql {
        ...
        static String inline(String sql, List<String> formattedValues) {
            ...
            for (int i=0;i<sql.length();i++){
                ...
                if(!inD && c=='\''){ out.append(c); if(!(n=='\'')) inS=!inS; continue; }
        ...
        static String inlineFormat(Object v, Vendor vendor) {
            ...
            if (v instanceof byte[]) {
                ...
                String hex=sb.toString(); return (vendor==Vendor.MYSQL||vendor==Vendor.POSTGRES?"X'":"0x") + hex + (b.length>max?"…":"") + (vendor==Vendor.MYSQL||vendor==Vendor.POSTGRES?"'":"");
            }
    ...
    static final class Log {
        ...
        static synchronized void init(String p){ path=p; Q=new LinkedBlockingQueue<>(); T=new Thread(Log::drain,"sql-agent-writer"); T.setDaemon(true); T.start(); Runtime.getRuntime().addShutdownHook(new Thread(()->{ try{ Q.put("__EOF__"); }catch(InterruptedException ignored){} })); }
        static void json(String line){ try{ Q.put(line); }catch(InterruptedException e){ Thread.currentThread().interrupt(); } }
        static void drain(){ try(BufferedWriter w=new BufferedWriter(new FileWriter(path, StandardCharsets.UTF_8,true))){ while(true){ String line=Q.take(); if("__EOF__".equals(line)) break; w.write(line); w.newLine(); w.flush(); } } catch(IOException|InterruptedException e){ throw new UncheckedIOException(e instanceof IOException?(IOException)e:new IOException(e)); } }
    }

    // utils
    static String esc(String s){ if(s==null) return ""; StringBuilder sb=new StringBuilder(s.length()+8); for(int i=0;i<s.length();i++){ char c=s.charAt(i); switch(c){ case '"': sb.append("\\\""); break; case '\\': sb.append("\\\\"); break; case '\n': sb.append("\\n"); break; case '\r': sb.append("\\r"); break; case '\t': sb.append("\\t"); break; default: if(c<0x20) sb.append(String.format("\\u%04x", (int)c)); else sb.append(c);} } return sb.toString(); }
