...
public final class UnifiedFinalSqlAgent {
    ...
    static final class Xf implements ClassFileTransformer {
        @Override public byte[] transform(ClassLoader loader, String className, Class<?> cls, ProtectionDomain pd, byte[] buf) {
            if (className == null) return null;
            String n = className.replace('/', '.');
            if (n.startsWith("java.") || n.startsWith("javax.") || n.startsWith("jdk.") || n.startsWith("sun.") || n.startsWith("com.sun.")) return null;
            ...
            try {
                ...
                try { Log.json("{\"type\":\"agent-error\",\"class\":\"" + esc(n) + "\",\"error\":\"" + esc(String.valueOf(t)) + "\"}"); } catch (Throwable ignored) {}
                return null;
            }
        }

        boolean subtypeOf(ClassPool cp, CtClass ct, String fqn) {
            try { return ct.subtypeOf(cp.get(fqn)); } catch (NotFoundException e) { return false; }
        }
        ...
        boolean instrumentMyBatis(CtClass ct) throws Exception {
            ...
            for (CtMethod m : ct.getDeclaredMethods()) {
                ...
                if ("update".equals(mn) && p.length >= 2 && p[0].getName().equals("org.apache.ibatis.mapping.MappedStatement")) { injectMyBatisAround(m); mod = true; }
                if ("query".equals(mn) && p.length >= 4 && p[0].getName().equals("org.apache.ibatis.mapping.MappedStatement")) { injectMyBatisAround(m); mod = true; }
            }
            return mod;
        }
            ...
            for (CtMethod m : ct.getDeclaredMethods()) {
                ...
                if (("prepareStatement".equals(name) || "prepareCall".equals(name))) {
                    ...
                    if (p.length >= 1 && p[0].getName().equals("java.lang.String")) {
                        ...
                        try { m.insertAfter("{ " + code + " }", false); mod = true; } catch (CannotCompileException ignored) {}
                    }
                }
            }
            return mod;
        ...
        boolean instrumentPreparedStatement(CtClass ct) throws Exception {
            ...
            for (CtMethod m : ct.getDeclaredMethods()) {
                ...
                if (name.startsWith("set") && p.length >= 1 && CtClass.intType.equals(p[0])) {
                    ...
                    try { m.insertBefore("{ " + code + " }"); mod = true; } catch (CannotCompileException ignored) {}
                }
                if ("clearParameters".equals(name) && p.length == 0) {
                    try { m.insertBefore("{ " + UnifiedFinalSqlAgent.class.getName() + ".JdbcCapture.clearParams($0); }"); mod = true; } catch (CannotCompileException ignored) {}
                }
                if ("addBatch".equals(name) && p.length == 0) {
                    try { m.insertBefore("{ " + UnifiedFinalSqlAgent.class.getName() + ".JdbcCapture.markBatch($0, null); }"); mod = true; } catch (CannotCompileException ignored) {}
                }
                if (name.startsWith("execute")) {
                    try { m.insertBefore("{ " + UnifiedFinalSqlAgent.class.getName() + ".onJdbcExecute($0, \"" + name + "\", $args); }"); mod = true; } catch (CannotCompileException ignored) {}
                }
                if ("close".equals(name) && p.length == 0) {
                    try { m.insertBefore("{ " + UnifiedFinalSqlAgent.class.getName() + ".JdbcCapture.clearAll($0); }"); mod = true; } catch (CannotCompileException ignored) {}
                }
            }
            return mod;
        }
        ...
        boolean instrumentStatement(CtClass ct) throws Exception {
            ...
            for (CtMethod m : ct.getDeclaredMethods()) {
                ...
                if ("addBatch".equals(name) && p.length == 1 && p[0].getName().equals("java.lang.String")) {
                    try { m.insertBefore("{ " + UnifiedFinalSqlAgent.class.getName() + ".JdbcCapture.markBatch($0, (String)$1); }"); mod = true; } catch (CannotCompileException ignored) {}
                }
                if (name.startsWith("execute")) {
                    try { m.insertBefore("{ " + UnifiedFinalSqlAgent.class.getName() + ".onJdbcExecute($0, \"" + name + "\", $args); }"); mod = true; } catch (CannotCompileException ignored) {}
                }
    ...
    static final class MyBatisCtx {
        ...
        static final class Ctx { String mapperId; String rawSql; String finalSql; long startNs; }
        ...
        static void enter(Object mappedStatement, Object paramObj) {
            ...
            try {
                ...
                if (meta != null) {
                    try { isSimple = (Boolean) thr.getClass().getMethod("hasTypeHandler", Class.class).invoke(thr, paramObj.getClass()); } catch (Throwable ignored) {}
                }
                ...
                for (Object pm : pms) {
                    ...
                    if (mode != null && mode.equals(OUT)) { values.add("NULL"); continue; }
                    ...
                    else { try { value = meta.getClass().getMethod("getValue", String.class).invoke(meta, prop); } catch (Throwable t) { value = null; } }
                ...
                try { Log.json("{\"type\":\"agent-warn\",\"msg\":\"mybatis-render-failed\",\"error\":\"" + esc(String.valueOf(t)) + "\"}"); } catch (Throwable ignored) {}
            }
        }
        ...
        static void exit() {
            ...
            String json = Json.start()
                    ...
                    .end();
            Log.json(json);
            c.mapperId = c.rawSql = c.finalSql = null; c.startNs = 0L;
        }
    }
    ...
    public static void onJdbcExecute(Object self, String method, Object[] args) {
        try {
            ...
            java.sql.Connection conn = null; try { conn = (self instanceof java.sql.Statement) ? ((java.sql.Statement)self).getConnection() : null; } catch (Throwable ignored) {}
            ...
            try { Log.json("{\"type\":\"agent-error\",\"when\":\"jdbc-execute\",\"error\":\"" + esc(String.valueOf(t)) + "\"}"); } catch (Throwable ignored) {}
        } finally {
            try { JdbcCapture.clearBatch(self); } catch (Throwable ignored) {}
        }
    }
    ...
    static final class JdbcCapture {
        ...
        static boolean isBatch(Object stmt) { return Boolean.TRUE.equals(STMT_BATCH.get(stmt)); }
    ...
    enum Vendor { MYSQL, POSTGRES, ORACLE, SQLSERVER, H2, SQLITE, UNKNOWN;
        static Vendor detect(java.sql.Connection c){
            try { if (c == null) return UNKNOWN; String url = c.getMetaData().getURL(); if (url==null) return UNKNOWN; String u=url.toLowerCase();
                if (u.startsWith("jdbc:mysql:")) return MYSQL; if (u.startsWith("jdbc:postgresql:")) return POSTGRES; if (u.startsWith("jdbc:oracle:")) return ORACLE;
                if (u.startsWith("jdbc:sqlserver:")) return SQLSERVER; if (u.startsWith("jdbc:h2:")) return H2; if (u.startsWith("jdbc:sqlite:")) return SQLITE; } catch (Throwable ignored) {}
            return UNKNOWN; }
    }

    static final class DriverSpecific {
        /** Container for JDBC credential info */
        static final class ConnInfo { String url; String user; String password; }

        /**
         * Unwraps common pool proxies (Hikari & Druid) to the underlying driver Statement.
         */
        static Object unwrapPool(Object obj) {
            try {
                ...
                for (int depth=0; depth<8 && cur!=null; depth++) {
                    ...
                    if (next == null) next = tryDelegateMethod(cur, "getTargetStatement", "getStatement", "getPreparedStatement", "getRawStatement", "getDelegate");
                    if (next == null) return cur;
                    cur = next;
                }
                return cur;
            } catch (Throwable t) { return obj; }
        }
        ...
        private static Object findDelegateField(Object obj) {
            for (Class<?> k = obj.getClass(); k != null && k != Object.class; k = k.getSuperclass()) {
                for (java.lang.reflect.Field f : k.getDeclaredFields()) {
                    Class<?> ft = f.getType();
                    if (java.sql.Statement.class.isAssignableFrom(ft)) { try { f.setAccessible(true); return f.get(obj); } catch (Throwable ignored) {} }
                }
            }
            // Try common accessors
            try { java.lang.reflect.Method m = obj.getClass().getDeclaredMethod("getTargetStatement"); m.setAccessible(true); return m.invoke(obj); } catch (Throwable ignored) {}
            try { java.lang.reflect.Method m = obj.getClass().getDeclaredMethod("getPreparedStatement"); m.setAccessible(true); return m.invoke(obj); } catch (Throwable ignored) {}
            try { java.lang.reflect.Method m = obj.getClass().getDeclaredMethod("getStatement"); m.setAccessible(true); return m.invoke(obj); } catch (Throwable ignored) {}
            return null;
        }

        /**
         * Attempt to render final SQL via driver-specific hooks (e.g., MySQL's asSql()),
         * or fall back to toString() if -Dsql.agent.driverToString=true.
         */
        static String tryRender(Object stmt, Vendor v) {
            try {
                ...
                if (v == Vendor.MYSQL) {
                    ...
                    if (m != null) { m.setAccessible(true); Object r = m.invoke(stmt); if (r != null) return String.valueOf(r); }
                    if (Boolean.getBoolean("sql.agent.driverToString")) {
                        String s = String.valueOf(stmt);
                        int idx = s.indexOf(": "); return idx > 0 ? s.substring(idx+2) : s;
                    }
        ...
        static java.lang.reflect.Method findNoArg(Class<?> c, String name) {
            for (Class<?> k=c; k!=null && k!=Object.class; k=k.getSuperclass()) {
                for (java.lang.reflect.Method m : k.getDeclaredMethods()) if (m.getName().equals(name) && m.getParameterCount()==0) return m;
            }
            return null;
        }
         ...
         * - URL & user via DatabaseMetaData
         * - Password only when sql.agent.export.password=true and retrievable (Druid supported)
         */
        static ConnInfo readConnInfo(java.sql.Connection c) {
            ...
            try {
                if (c != null) {
                    ...
                    if (md != null) {
                        try { ci.url = md.getURL(); } catch (Throwable ignored) {}
                        try { ci.user = md.getUserName(); } catch (Throwable ignored) {}
                    }
                }
            } catch (Throwable ignored) {}
            ...
            if (wantPwd && c != null) {
                try {
                    ...
                    if (cn.startsWith("com.alibaba.druid.pool.")) {
                        Object ds = null;
                        // try getDirectDataSource() first
                        try { java.lang.reflect.Method m = c.getClass().getMethod("getDirectDataSource"); m.setAccessible(true); ds = m.invoke(c); } catch (Throwable ignored) {}
                        // or go through holder -> dataSource
                        if (ds == null) {
                            Object holder = null;
                            try { java.lang.reflect.Method m = c.getClass().getMethod("getConnectionHolder"); m.setAccessible(true); holder = m.invoke(c); } catch (Throwable ignored) {}
                            if (holder == null) holder = findFieldByName(c, "holder", "connectionHolder");
                            if (holder != null) {
                                Object ds2 = findFieldByTypeName(holder, "DruidAbstractDataSource", "DruidDataSource");
                                if (ds2 == null) ds2 = findFieldByName(holder, "dataSource");
                                ds = ds2;
                            }
                        }
                        if (ds != null) {
                            try { java.lang.reflect.Method gm = ds.getClass().getMethod("getPassword"); gm.setAccessible(true); Object p = gm.invoke(ds); if (p != null) ci.password = String.valueOf(p); } catch (Throwable ignored) {}
                        }
                    }
                } catch (Throwable ignored) {}
            }
    ...
    static final class Sql {
        ...
        static String inline(String sql, List<String> formattedValues) {
            ...
            int vi = 0; boolean inS=false,inD=false,inLC=false,inBC=false;
            for (int i=0;i<sql.length();i++){
                char c=sql.charAt(i); char n=(i+1<sql.length()?sql.charAt(i+1):'\0');
                if(!inS && !inD && !inBC && c=='-' && n=='-'){ inLC=true; out.append(c); continue; }
                if(inLC){ out.append(c); if(c=='\n') inLC=false; continue; }
                if(!inS && !inD && !inLC && c=='/' && n=='*'){ inBC=true; out.append(c); continue; }
                if(inBC){ out.append(c); if(c=='*' && n=='/'){ out.append(n); i++; inBC=false; } continue; }
                if(!inD && c=='\''){ out.append(c); if(inS){ if(n=='\''){ out.append(n); i++; } else { inS=false; } } else { inS=true; } continue; }
                if(!inS && c=='"'){ inD=!inD; out.append(c); continue; }
                if(!inS && !inD && c=='?' && vi<formattedValues.size()){ out.append(formattedValues.get(vi++)); continue; }
                out.append(c);
            }
            return out.toString();
        }
        ...
        static String inlineFormat(Object v, Vendor vendor) {
            ...
            if (v instanceof byte[]) {
                ...
                byte[] b=(byte[])v; StringBuilder sb=new StringBuilder();
            ...
            if (v instanceof java.sql.Array) {
                try { Object arr=((java.sql.Array)v).getArray(); if(arr instanceof Object[]){ Object[] a=(Object[])arr; List<String> parts=new ArrayList<>(); for(Object o:a) parts.add(inlineFormat(o,vendor)); return '('+String.join(", ",parts)+')'; } } catch(Throwable ignored){}
                return "ARRAY[...]";
            }
            if (v.getClass().isArray()) {
                int len = java.lang.reflect.Array.getLength(v); List<String> parts = new ArrayList<>(len);
                for (int i=0;i<len;i++) parts.add(inlineFormat(java.lang.reflect.Array.get(v,i), vendor));
                return '(' + String.join(", ", parts) + ')';
            }
            if (v instanceof Iterable) { List<String> parts=new ArrayList<>(); for(Object o:(Iterable<?>)v) parts.add(inlineFormat(o,vendor)); return '('+String.join(", ",parts)+')'; }
    ...
    static final class Json {
        final StringBuilder sb = new StringBuilder(); boolean first = true;
        static Json start(){ Json j=new Json(); j.sb.append('{'); return j; }
        Json kv(String k, String v){ return kvRaw(k, v==null?null:esc(v)); }
        Json kvRaw(String k, String raw){ if(!first) sb.append(','); first=false; sb.append('"').append(esc(k)).append('"').append(':'); if(raw==null) sb.append("null"); else if(isRaw(raw)) sb.append(raw); else sb.append('"').append(raw).append('"'); return this; }
        String end(){ return sb.append('}').toString(); }
        static boolean isRaw(String r){ return r.startsWith("{")||r.startsWith("[")||r.equals("true")||r.equals("false")||r.matches("^-?\\d+(\\.\\d+)?$"); }
        static String params(Map<Integer,Object> params){ StringBuilder p=new StringBuilder(); p.append('{'); boolean f=true; for(Map.Entry<Integer,Object> e:params.entrySet()){ if(!f) p.append(','); f=false; p.append('"').append(e.getKey()).append('"').append(':'); Object v=e.getValue(); if(v==null) p.append("null"); else if(v instanceof Number||v instanceof Boolean) p.append(String.valueOf(v)); else p.append('"').append(esc(String.valueOf(v))).append('"'); } p.append('}'); return p.toString(); }
    }

    static final class Log {
        static String path; private static BlockingQueue<String> Q; private static Thread T;
        static synchronized void init(String p){ path=p; try { java.io.File f=new java.io.File(p); java.io.File dir=f.getParentFile(); if(dir!=null) dir.mkdirs(); } catch(Throwable ignored){} Q=new LinkedBlockingQueue<>(); T=new Thread(Log::drain,"sql-agent-writer"); T.setDaemon(true); T.start(); Runtime.getRuntime().addShutdownHook(new Thread(()->{ try{ Q.put("__EOF__"); }catch(InterruptedException ignored){} })); }
        static void json(String line){ try{ Q.put(line); }catch(InterruptedException e){ Thread.currentThread().interrupt(); } }
        static void drain(){ try(BufferedWriter w=new BufferedWriter(new FileWriter(path, StandardCharsets.UTF_8,true))){ while(true){ String line=Q.take(); if("__EOF__".equals(line)) { // drain remaining
            List<String> rest = new ArrayList<>(); Q.drainTo(rest);
            for(String l:rest){ w.write(l); w.newLine(); }
            w.flush();
            break; }
            w.write(line);
            w.newLine();
