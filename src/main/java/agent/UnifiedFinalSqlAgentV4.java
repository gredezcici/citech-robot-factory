 ...
 */
public final class UnifiedFinalSqlAgentV4 {
    private UnifiedFinalSqlAgentV4() {
    }
    ...
    static final class Xf implements ClassFileTransformer {
        ...
        boolean instrumentDataSource(CtClass ct) throws Exception {
            ...
            for (CtMethod m : ct.getDeclaredMethods()) {
                ...
                try { if (!"java.sql.Connection".equals(m.getReturnType().getName())) continue; } catch (NotFoundException ignore) {}
                CtClass[] p = m.getParameterTypes();
                // getConnection()
                if (p.length == 0) {
                    try {
