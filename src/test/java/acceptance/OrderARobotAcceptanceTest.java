...
@SpringBootTest(
        webEnvironment = WebEnvironment.RANDOM_PORT, classes = {CitechRobotFactoryApplication.class}
)
public class OrderARobotAcceptanceTest {
    ...
    void should_order_a_robot() {
        this.postOrder("{ \n" +
                "                        \"components\": [\"I\",\"A\",\"D\",\"F\"]\n" +
                "                    }").then().assertThat().statusCode(HttpStatus.CREATED.value()).body("order_id", CoreMatchers.notNullValue(), new Object[0])
    ...
    void should_not_allow_GET_method_on_orders() {
        this.getReq("7da588a6-c766-46ce-ad62-431d08ed6995").then().assertThat().statusCode(HttpStatus.METHOD_NOT_ALLOWED.value());
    }
    ...
    void should_not_allow_wrong_media_type_on_orders() {
        this.postOrderWithText("{ \n" +
                "                        \"components\": [\"I\",\"A\",\"D\",\"F\"]\n" +
                "                    }").then().assertThat().statusCode(HttpStatus.UNSUPPORTED_MEDIA_TYPE.value());
    }
    ...
    private Response getReq(String orderid) {
        return RestAssured.given()
                ...
                .param("id", orderid)
                .when().port(this.getSpringBootPort())
                .get("/orders")
                .then()
                .extract().response();
    }
    ...
    private Response postOrder(String body) {
        return RestAssured.given().body(body).contentType(ContentType.JSON).when().port(this.getSpringBootPort())
                .post("/orders", new Object[0]);
    }

    private Response postOrderWithText(String body) {
        return RestAssured.given().body(body).contentType(ContentType.TEXT).when().port(this.getSpringBootPort())
                .post("/orders", new Object[0]);
    }
}
